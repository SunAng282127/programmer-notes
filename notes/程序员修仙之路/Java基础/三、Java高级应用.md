...
2. 方法引用的理解
   ...
   - 当需要提供一个函数式接口的实例时，就可以使用lambda表达式提供此实例。当满足一定条件的情况下，还可以使用方法引用或构造器引用替换lambda表达式

3. 方法引用的本质

   - 方法引用作为了函数式接口的实例
   - 方法引用适用于相当于Lambda表达式中只写一行代码的情况
...
5. 方法引用的具体使用情况说明
   ...
   - 对象::实例方法。此方法要求函数式接口中的抽象方法a与其内部实现时调用的对象的某个非静态方法b的形参列表和返回值类型都相同（或一致，满足装箱拆箱或多态即可），此时可以考虑使用方法b实现对方法a的替换、覆盖，此替换或覆盖即为方法引用。此方法b是非静态方法，需要对象来调用
   - 类::静态方法。此方法要求函数式接口中的抽象方法a与其内部实现时调用的对象的某个静态方法b的形参列表和返回值类型都相同（或一致，满足装箱拆箱或多态即可），此时可以考虑使用方法b实现对方法a的替换、覆盖，此替换或覆盖即为方法引用。此方法b是静态方法，需要类来调用
   - 类::实例方法。此函数式接口中的抽象方法a与其内部实现调用对象的某个方法b的返回值类型相同。同时抽象方法a中有n个参数，方法b中有n-1个参数，且抽象方法a的第一个参数作为方法b的调用者，且抽象方法a的后n-1个参数与方法b的n-1个参数类型相同（或一致，满足装箱拆箱或多态即可），则此替换或覆盖即为方法引用。此方法b是非静态方法，需要对象调用，但是形式上要写出对象a所属的类
