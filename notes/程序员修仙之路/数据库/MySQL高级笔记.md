...
- 如果一个事务在插入一条记录时需要判断一下插入位置是不是被别的事物加了间隙锁（临键锁也包含了间隙锁），如果有的话插入操作需要等待，直到拥有间隙锁的那个事物提交。但是InnoDB规定事物在等待的时候也需要在内存中生成一个锁结构，表明有事物想在某个间隙中插入新记录，但是现在在等待，这样等待的时候生成的锁结构就叫做插入意向锁
- 插入意向锁是在插入一条记录行前，由INSERT操作产生的一种间隙锁。插入意向锁是一种间隙锁，不是意向锁
- 插入意向锁并不会阻止别的事物继续获取该记录上任何类型的锁。插入意向锁与插入意向锁之间也不会有冲突
